<script lang="ts">
    ...
    $effect(() => {
      ...
      // Add package price
      const packageItem = PACKAGES.find(p => p.name === masterStore.packageInfo.selection);
      if (packageItem) {
        items.push({
      ...
      masterStore.scheduleDays.forEach((day, dayIndex) => {
    ...
    function selectPackage(packageName: string) {
      masterStore.updatePackageInfo({ selection: packageName });
    }
  </script>
  ...
  <div class="memorial-calculator">
    ...
    <div class="package-selection">
      {#each PACKAGES as pkg}
        <button 
          class="px-4 py-2 rounded-lg border-2 {masterStore.packageInfo.selection === pkg.name ? 'border-primary bg-primary/10' : 'border-gray-200'}"
    ...
    <div class="schedule-days mt-6 space-y-8">
      {#each masterStore.scheduleDays as day, dayIndex}
        <div class="day p-4 border rounded-lg">
          <div class="flex items-center justify-between mb-4">
            ...
            {#if masterStore.scheduleDays.length > 1}
              <button 
                on:click={() => masterStore.removeScheduleDay(dayIndex)}
          ...
          <div class="locations space-y-4">
            {#each day.locations as location, locationIndex}
              <div class="location p-4 border rounded-lg">
                <div class="flex items-center justify-between mb-4">
                  ...
                  {#if locationIndex > 0}
                    <button 
                      on:click={() => masterStore.removeScheduleDayLocation(dayIndex, locationIndex)}
                      class="text-sm text-red-600 hover:text-red-700"
                    >
                      Remove Location
                    </button>
                ...
                <input 
                  ...
                  on:input={() => masterStore.updateScheduleDayLocation(dayIndex, locationIndex, { name: location.name })}
                ...
                <input 
                  ...
                  on:input={() => masterStore.updateScheduleDayLocation(dayIndex, locationIndex, { address: location.address })}
                ...
                <div class="grid grid-cols-2 gap-4 mt-2">
                  <label class="block">
                    ...
                    <input 
                      ...
                      on:input={() => masterStore.updateScheduleDayLocation(dayIndex, locationIndex, { startTime: location.startTime })}
                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm"
                    />
                  </label>
                  ...
                  <label class="block">
                    ...
                    <input 
                      ...
                      on:input={() => masterStore.updateScheduleDayLocation(dayIndex, locationIndex, { duration: location.duration })}
                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm"
                    />
                  </label>
                </div>
                ...
                <label class="mt-2 block">
                  ...
                  <textarea
                    ...
                    on:input={() => masterStore.updateScheduleDayLocation(dayIndex, locationIndex, { notes: location.notes })}
                    rows="2"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm"
                  ></textarea>
                </label>
                ...
                <label class="mt-2 flex items-center">
                  <input 
                    ...
                    on:change={() => masterStore.updateScheduleDayLocation(dayIndex, locationIndex, { travelExceedsHour: location.travelExceedsHour })}
            ...
            {#if day.locations.length < 3}
              <button 
                on:click={() => masterStore.addScheduleDayLocation(dayIndex)}
      ...
      <button 
        on:click={() => masterStore.addScheduleDay()}
    ...
    <div class="cart-summary mt-8 p-4 bg-gray-50 rounded-lg">
      ...
      <div class="mt-4 space-y-2">
        {#each masterStore.packageInfo.items || [] as item}
        ...
        <div class="pt-4 border-t flex justify-between font-semibold">
          <span>Total:</span>
          <span>${masterStore.packageInfo.priceTotal}</span>
        </div>
      </div>
    </div>
    ...
    <div class="actions mt-6 flex gap-4">
      ...
      <form 
        ...
        class="flex-1"
      >
        <input type="hidden" name="selectedPackage" value={masterStore.packageInfo.selection} />
        <input type="hidden" name="cartItems" value={JSON.stringify(masterStore.packageInfo.items)} />
        <input type="hidden" name="cartTotal" value={masterStore.packageInfo.priceTotal} />
        <input type="hidden" name="scheduleDays" value={JSON.stringify(masterStore.scheduleDays)} />
