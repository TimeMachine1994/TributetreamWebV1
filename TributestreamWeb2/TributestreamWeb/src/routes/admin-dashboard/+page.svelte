<script lang="ts">
    ...
    // Store for tributes
    let tributes = writable<Tribute[]>([]);
  
    // State to track the tribute being edited
    let editingId = $state<string | null>(null);
    let editingHtml = $state<string>("");
    ...
    const fetchTributes = async () => {
        ...
        tributes.set(data.tributes || []);
    };

    // Save updated custom_html
    const saveCustomHtml = async (tributeId: string) => {
        try {
            const token = localStorage.getItem('jwt_token');
            console.log('🔑 Token retrieved for HTML save:', token ? 'Present' : 'Missing');
            
            const response = await fetch(`/api/admin/tributes/${tributeId}/custom-html`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
