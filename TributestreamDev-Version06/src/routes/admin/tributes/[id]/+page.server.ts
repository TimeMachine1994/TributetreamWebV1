...
export const load = (async ({ params, fetch }) => {
  console.log('🔍 Loading tribute:', params.id);
  ...
  // Fetch tribute data
  const tributeResponse = await fetch(`/api/tributes/${params.id}`);
  if (!tributeResponse.ok) {
    console.error('❌ Failed to load tribute:', tributeResponse.statusText);
    error(tributeResponse.status, 'Failed to load tribute');
  }
...
export const actions = {
  update: async ({ request, params, fetch }) => {
    console.log('📝 Updating tribute:', params.id);
    ...
    console.log('📦 Update data:', tributeData);

    try {
      const response = await fetch(`/api/tributes/${params.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        console.error('❌ Failed to update tribute:', response.statusText);
  ...
  delete: async ({ params, fetch }) => {
    console.log('🗑️ Deleting tribute:', params.id);
    ...
    try {
      ...
      if (!response.ok) {
        console.error('❌ Failed to delete tribute:', response.statusText);
        return fail(response.status, {
