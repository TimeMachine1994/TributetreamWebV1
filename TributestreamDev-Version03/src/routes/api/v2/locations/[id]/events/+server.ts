/**
 * Events by Location API endpoints for the API v2
 */
...
/**
 * GET handler for retrieving a list of events by location
 * 
 * @route GET /api/v2/locations/:id/events
 * @param request The request object
 * @returns Response with paginated list of events for a location
 */
export const GET: RequestHandler = async ({ params, url, cookies }) => {
  ...
  const locationData = await wpClient.get<Record<string, unknown> | null>(`wp/v2/locations/${locationId}`)
    .catch(() => null);
  ...
  queryString.append('location_id', locationId);
